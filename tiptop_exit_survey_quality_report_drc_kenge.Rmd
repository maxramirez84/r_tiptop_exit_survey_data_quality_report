...
ProgressOfArea(data, study.area.column, study.area.label, interval = 10, 
               approaches.mean = 30, lang = language)
```
...
for (hf in sort(unique(data[[column]]))) {
  ...
  print(subtitle)
  SurveyProfileOfHealthFacility(data, study.area.column, hf, language)
}
```
...
DuplicatedWomen(data, study.area.column, study.area.label, language)
```
...
DuplicatedRecords(data, study.area.column, study.area.label, language)
```
...
mip.rate <- MiPRate(data)
mip.hosp.rate <- HospitalizedMiPRate(data)
```
