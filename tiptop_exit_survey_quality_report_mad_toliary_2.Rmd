...
api.token.exit.survey <- kAPITokenMADES
# TIPTOP Cost-Effectiveness Health System DRC
api.token.health.systems <- kAPITokenMADHS 
...
# Partner information
partner.name <- "MANISA"
partner.url  <- "http://www.manisa.mg"

# Study area description
country           <- "Madagascar"
study.area.id     <- 2
study.area.label  <- "Toliary 2"
study.area.column <- "toliary_2"
sample.size       <- 142
hf.to.be.surveyed <- 13
...
for (hf in sort(unique(exit.survey.data[[column]]))) {
  ...
  SurveyProfileOfHealthFacility(exit.survey.data, health.system.data, 
                                study.area.column, hf, language, less.than = 15)
}
```
